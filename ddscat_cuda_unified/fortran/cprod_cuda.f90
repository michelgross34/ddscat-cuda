...
#ifdef DDSCAT_CUDA_MATVEC
      USE CUDA_MATVEC_BRIDGE,ONLY : CUDA_CPROD
#endif
      IMPLICIT NONE
...
!***********************************************************************
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 1: matvec, IPBC=',IPBC
!      write(0,*)'               about to call CPROD with CWHAT=N'
!***
#ifdef DDSCAT_CUDA_MATVEC
      CALL CUDA_CPROD(AK_TF,GAMMA,DX,CMDFFT,'N',CXADIA,CXAOFF,CXX,CXY,CXZC,CXZW, &
...
#endif
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 2: matvec'
!***
      RETURN
...
!***********************************************************************
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 3: entered cmatvec with IPBC=',IPBC
!***
#ifdef DDSCAT_CUDA_MATVEC
      CALL CUDA_CPROD(AK_TF,GAMMA,DX,CMDFFT,'C',CXADIA,CXAOFF,CXX,CXY,CXZC,CXZW, &
                 IDVOUT,IOCC,MXN3F,MXNATF,MXNXF,MXNYF,MXNZF,NAT,NAT0,NAT3,NX, &
                 NY,NZ,IPBC,PYD,PZD)
#else
...
#endif
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 4: cmatvec'
!***
      RETURN
...
!-----------------------------------------------------------------------
!*** diagnostic
!      write(0,fmt='(a,4i4)')'cprod_v2 ckpt 5 with nx,ny,nz,ipbc=',nx,ny,nz,ipbc
!      write(0,*)'                cwhat=',cwhat
!***
...
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 6: akd=',akd
!***
...
!*** diagnostic
!         write(0,*)'cprod_v2 ckpt 7: about to call eself from cprod'
!         write(0,*)'   j     cxpol(j)'
!         do j1=1,nat3
!            write(0,fmt='(a,i5,2f10.3)')'cprod_v2 ckpt 7',j1,cxx(j1)
!         enddo
!***
!***
...
!*** diagnostic
!         write(0,*)'cprod_v2 ckpt 8'
!         write(0,*)'   j     cxe(j)'
!         do j1=1,nat3
!            write(0,fmt='(a,i4,2f10.3)')'cprod_v2 ckpt 8:',j1,cxy(j1)
!         enddo
!***
!***
...
!*** diagnostic
!        write(0,*)'cprod_v2 ckpt 9'
!        j2=0
!        do j1=1,nat3
!           if(.not.(abs(cxx(j1))>=0.e0).or.(abs(cxx(j1))>1.e10))then
!              write(0,*)'j1=',j1,' cxx(j1)=',cxx(j1)
!              j2=j2+1
!           endif
!           if(.not.(abs(cxy(j1))>=0.e0).or.(abs(cxy(j1))>1.e10))then
!              write(0,*)'j1=',j1,' cxy(j1)=',cxy(j1)
!              j2=j2+1
!           endif
!           if(.not.(abs(cxadia(j1))>=0.e0).or.(abs(cxadia(j1))>1.e10))then
!              write(0,*)'j1=',j1,' cxadia(j1)=',cxadia(j1)
!              j2=j2+1
!           endif
!        enddo
!        write(0,*)'cprod_v2 ckpt 10: cxadia,cxx,cxy checked for NaN or > 1.e10'
...
!*** diagnostic
!            write(0,fmt='(a,i6,a,1p6e8.1)')'cprod_v2 ckpt 11, j1=',j1, &
!               ' cxadia,cxx,cxy=',cxadia(j1),cxx(j1),cxy(j1)
!***
            CXY(J1)=CXADIA(J1)*CXX(J1)-CXY(J1)
         ENDDO

!*** diagnostic
!        write(0,*)'cprod_v2 ckpt 12'
!        do j1=1,nat3
...
! 97.12.28 (BTD):
         ...
         ENDDO
!*** diagnostic
!        write(0,*)'cprod_v2 ckpt 13, check cxy for NaN...'
!        do j1=1,nat3
!           if(.not.(abs(cxy(j1))>=0.d0))then
!              write(0,*)'j1=',j1,' cxy(j1)=',cxy(j1)
!           endif
...
!*** diagnostic
!      write(0,*)'cprod_v2 ckpt 14, check xy for NaN'
!      j2=0
!      do j1=1,nat3
!         if(.not.(abs(cxy(j1))>=0.d0))then
!            j2=j2+1

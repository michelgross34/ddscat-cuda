...
! v6 flatau added 
      ...
      USE CUDA_MATVEC_BRIDGE,ONLY : CUDA_GPBICG_SOLVE,CUDA_QMRCCG_SOLVE,CUDA_PBCGST_SOLVE,CUDA_PBCGS2_SOLVE,CUDA_PETRKP_SOLVE, &
           CUDA_BICGST4_SOLVE,CUDA_GPBGS2_SOLVE,CUDA_GPBGS4_SOLVE,CUDA_EVALQ_GPU,CUDA_SCAT_GPU
#endif
      IMPLICIT NONE
! v6 flatau added
      TYPE(CGSTRUCT) CG
...
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_PETRKP_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
...
! PJF 2013 new implementation of PETRKP (good old horse...)
               ...
               TIMERS(1)=DTIME
#ifdef DDSCAT_CUDA_SOLVERS
               TIMERS(2)=REAL(MULTIPLICATIONS,KIND=WP)
#else
...
!***
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_PBCGS2_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
               MXMATVEC=MULTIPLICATIONS
               IPAR(12)=0
#else
...
! BiCGStab(2) is the IFDDA/ADDA BCGS2 alias. Reuse the existing CUDA ZBCG2 L=2 path.
               CALL CINIT(NAT3,CMPLX(0._WP,0._WP,KIND=WP),CXPOL_TF,1)
               TOLR=TOL
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_PBCGS2_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    ...
                    TOLR,ITERN,MULTIPLICATIONS)
#else
               STOP 'BICGS2 is available in ddscat_cuda build only'
#endif
            ...
            ELSEIF(CMDSOL=='BICGS4')THEN
               ...
               TOLR=TOL
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_BICGST4_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
               STOP 'BICGS4 is available in ddscat_cuda build only'
#endif
            ...
            ELSEIF(CMDSOL=='GPBGS2')THEN
               ...
               TOLR=TOL
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_GPBGS2_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
               STOP 'GPBGS2 is available in ddscat_cuda build only'
#endif
            ...
            ELSEIF(CMDSOL=='GPBGS4')THEN
               ...
               TOLR=TOL
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_GPBGS4_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
               STOP 'GPBGS4 is available in ddscat_cuda build only'
#endif
...
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_GPBICG_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
...
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_QMRCCG_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
...
#ifdef DDSCAT_CUDA_SOLVERS
               CALL CUDA_PBCGST_SOLVE(AK_TF,GAMMA,DX,CMDFFT,CXADIA,CXAOFF, &
                    CXE_TF,CXPOL_TF,CXSCR1,CXZC,CXZW,IDVOUT,IOCC,MXN3,MXNAT, &
                    MXNX,MXNY,MXNZ,NAT,NAT0,NAT3,NX,NY,NZ,IPBC,TOL,MXITER, &
                    TOLR,ITERN,MULTIPLICATIONS)
#else
...
#ifdef DDSCAT_CUDA_SOLVERS
            CALL CUDA_EVALQ_GPU(CXADIA,CXAOFF,AK_TF,NAT03,E02,CXE_TF,CXPOL_TF,CABS,CEXT,CPHA,.TRUE.)
#else
...
#ifdef DDSCAT_CUDA_SOLVERS
               IF(CMDTRQ=='NOTORQ')THEN
                  CALL CUDA_SCAT_GPU(AK_TF,AKS_TF,DX,EM1_TF,EM2_TF,E02,ETASCA,CXE01_TF,CBKSCA,CSCA,CSCAG,CSCAG2, &
                                     CXF11,CXF21,MXSCA,JPBC,NAVG,NSCAT,X0)
               ELSE
...
!***
               ...
               IF(CMDTRQ=='NOTORQ')THEN
                  CALL CUDA_SCAT_GPU(AK_TF,AKS_TF,DX,EM1_TF,EM2_TF,E02,ETASCA,CXE01_TF,CBKSCA,CSCA,CSCAG,CSCAG2, &
                                     CXF12,CXF22,MXSCA,JPBC,NAVG,NSCAT,X0)
               ELSE
                  CALL SCAT(AK_TF,AKS_TF,DX,EM1_TF,EM2_TF,E02,ETASCA,CMDTRQ,   &
                            CBKSCA,CSCA,CSCAG,CSCAG2,CTRQAB,CTRQSC,CXE_TF,     &
...
#ifdef DDSCAT_CUDA_SOLVERS
         CALL CUDA_EVALQ_GPU(CXADIA,CXAOFF,AK_TF,NAT03,E02,CXE_TF,CXPOL_TF,CABS,CEXT,CPHA,.FALSE.)
#else
         CALL EVALQ(CXADIA,CXAOFF,AK_TF,NAT03,E02,CXE_TF,CXPOL_TF, &
                    CABS,CEXT,CPHA,MXN3,1)
#endif

...
! 16.08.25 (BTD): following suggestion from Sergey Ph. Ruzankin,
!                 MPI initialization has been moved from subroutine
!                 DDSCAT to MAIN
...
! initialize NRFLD to 0 before first call to DDSCAT
! NB: the variable NRFLD used by DDSCAT is *not* identical to the
...
!                                          set NRFLD=0
!                                     = 1, then nearfield E only
!                                          set NRFLD=1 and NRFLDB=0
!                                          and store polarizations
!                                     = 2, then nearfield E and B
!                                          set NRFLD=1 and NRFLDB=1
!                                          and store polarizations
...
!                      
      ...
      IF(IARGC().EQ.1)THEN
         ...
         CFLPAR=COMMAND(1)
      ENDIF
      WRITE(0,FMT='(A)')'>DDSCAT using parameter file='
      WRITE(0,FMT='(A)')'        ',CFLPAR
      NRFLD=0
      AK2OLD=-999._WP
      AK3OLD=-999._WP
      WOLD=-999._WP
...
!-------------------------------- v7.3.3 -------------------------------
      ...
      USE DDCOMMON_0,ONLY: CFLPAR,IDIPINT
...
! 13.06.24 (BTD) * minor modification to format 9031 to avoid output
!                  overflow for large dielectric functions
! 13.12.28 (BTD) v7.3.1
!                * added NRFLD to argument list of SHARE1
!                  (thanks to Sergey Ph. Ruzankin for pointing this out)
! 14.01.06 (BTD) * added NORICHAR to argument list of SHARE1
!                  (thanks to Mike Wolff for pointing this out)
! 14.10.11 (BTD) * add SAVE E1A,E2A,WVA to retain arrays when
!                  doing nearfield calculations with NCOMP=1
! 15.03.12 (BTD) * change notation for x,y,z lab vectors in target frame
!                  XLR -> XL_TF
!                  YLR -> YL_TF
!                  ZLR -> YL_TF
!                  A1  -> A1_TF
!                  A2  -> A2_TF
!                  A3  -> A3_TF
! 15.04.04 (BTD) * add XL_TF,YL_TF,ZL_TF to argument list of ROTATE
! 15.04.05 (BTD) * remove code to calculate XL_TF,YL_TF,ZL_TF in DDSCAT
! 15.04.06 (BTD) * add MYID to argument list of ROTATE
!                * add AK1 to argument list of SHARE1
! 15.09.29 (BTD) * move definition of MXNATF,MXNXF,MXNYF,MXNZF,MXN3F
!                  out of MYID=0 section so that it will be executed
!                  by MPI slave processes as well
! 16.08.25 (BTD) v7.3.2
!                * following suggestion from Sergey Ph. Ruzankin,
!                  DDSCAT initialization and finalization have been
!                  moved from subroutine DDSCAT to MAIN,
!                  so that DDSCAT initialization and finalization
...
!                  more than one PHI value used: when doing nearfield
!                  calculations, need to call WRITEPOL for all PHI values
!                  (problem encountered by Yongmei Wang).
! 23.01.03 (BTD) * added calculation of XCM(1-3)
!                  XCM(1-3) = location of target centroid
!                * add output to log file with XCM(1-3)
! end history
...
!      PARAMETER(MXCOMP=256)
      ...
      PARAMETER(MXRAD=1000,MXWAV=1000,MXWAVT=15000)
...
!                     be printed out (e.g., 3 for element S_{13})
      ...
      CHARACTER ::                              &
         ...
         CFLLOG*14,CFLSHP*90,CSHAPE*9,CSTAMP*26 !
      CHARACTER(60) :: &
        CFLEPS(MXCOMP) !
      COMPLEX(WP) :: &
        CXZERO       !
...
! A2_TF(1-3)=direction of target axis a2 in Target Frame
!           =initial direction of target axis a2 in Lab Frame
! A3_TF(1-3)=direction of target axis a3=a1xa2 in Target Frame
...
! XL_TF(3)=unit vector xlab in target frame
! YL_TF(3)=unit vector ylab in target frame
! ZL_TF(3)=unit vector zlab in target frame
...
! SM(4,4,1-NSCAT)=Mueller scattering matrix
! SMORI(4,4,1-NSCAT)=orientation-averaged Mueller matrix
! SMORI_1(4,4,1-NSCAT)=Mueller matrix for single orientation
...
! External subroutines:
      ...
      EXTERNAL COLSUM,DIELEC,ERRMSG,EXTEND,GETFML,GETMUELLER,        &
               NAMER,NAMID,NEARFIELD,ORIENT,PBCSCAVEC,REAPAR,ROTATE, &
               SCAVEC,SHARE0,SHARE1,SHARE2,TARGET,VERSION,           &
               WRIMSG,WRITEFML,WRITEPOL,WRITESCA                     !
...
!=======================================================================
      ...
      IF(MYID==0)THEN   ! begin if(myid==0)... #1
         WRITE(IDVOUT,FMT='(A,A)')'>DDSCAT --- ',CSTAMP
         WRITE(IDVOUT,FMT='(A,I6)')'>DDSCAT     VERSNUM=',VERSNUM
#ifdef openmp
         WRITE(IDVOUT,FMT='(A,5X,A)')'>DDSCAT', &
...
! only master thread does this:
         IF(TID.EQ.0)THEN
            ...
            WRITE(CMSGNM,FMT='(4X,A,I3)')'Number of OpenMP threads =',NTHREADS
            CALL WRIMSG('DDSCAT',CMSGNM)
         ENDIF
...
!$OMP END PARALLEL
         ...
         IF(WP==KIND(0.E0))THEN
            WRITE(CMSGNM,FMT='(4X,A)')'Single-precision version'
...
! CWORD = length (bytes) of COMPLEX word
         ...
         ELSEIF(WP==KIND(0.D0))THEN
            WRITE(CMSGNM,FMT='(4X,A)')'Double-precision version'
            NRWORD=8.
            RWORD=8._WP
            CWORD=2._WP*RWORD
         ELSE
...
! BTD: I cannot really see why the following is beneficial...
...
! A1_TF(1-3)=target axis 1 in Target Frame
! A2_TF(1-3)=target axis 2 in Target Frame
...
!*** diagnostic
!            write(0,*)'ddscat ckpt 8.1'
!***
            CALL TARGET(A1_TF,A2_TF,CSHAPE,CFLSHP,IDVSHP,IOSHP,CDESCR,MXNAT,   &
                        SHPAR,DX,NAT0,IXYZ0,ICOMP,IDVOUT,NAT03,PYD,PZD,BETADF, &
                        PHIDF,THETADF,X0,IANISO,NCOMP_NEED)
!*** diagnostic
!            write(0,*)'ddscat ckpt 8.2, myid,cdescr=',myid,cdescr
...
! determine coordinates of centroid

            DO J=1,3
               XCM(J)=0.
               DO JJ=1,NAT0
                  XCM(J)=XCM(J)+REAL(IXYZ0(JJ,J))
               ENDDO
               XCM(J)=X0(J)+XCM(J)/REAL(NAT0)
            ENDDO
...
! write out some information on target geometry
         ...
         CALL WRIMSG('DDSCAT',CMSGNM)
         WRITE(CMSGNM,FMT='(A)')'X0(1-3) = location/d of lattice site (0,0,0)'
         CALL WRIMSG('DDSCAT',CMSGNM)
         WRITE(CMSGNM,FMT='(3F10.5,A)')X0,' = X0(1-3)'
         CALL WRIMSG('DDSCAT',CMSGNM)
         WRITE(CMSGNM,FMT='(A)')'XCM(1-3) = location/d of centroid'
         CALL WRIMSG('DDSCAT',CMSGNM)
         WRITE(CMSGNM,FMT='(3F10.5,A)')XCM,' = XCM(1-3)'
         CALL WRIMSG('DDSCAT',CMSGNM)
         WRITE(CMSGNM,FMT='(A)')                                       &
...
!       y_TF  = (IY+X0(2))*d*DX(2)
!       z_TF  = (IZ+X0(3))*d*DX(3)
!  A1_TF(1-3) = target axis A1 in TF
!  A2_TF(1-3) = target axis A2 in TF
...
!               dimension ICOMP(3*MXNAT)

!               J = 1 - NAT0                   =comp for 1x,2x,3x,..,NAT0x
!               J = NAT0+1 - MXNAT             =0
!               J = MXNAT+1 - MXNAT+NAT0       =comp for 1y,2y,3y,..,NAT0y
!               J = MXNAT+NAT0+1 - 2*MXNAT     =0
!               J = 2*MXNAT+1 - 2*MXNAT+NAT0   =comp for 1z,2z,3z,...,NAT0z
!               J = 2*MXNAT+NAT0+1 - 3*MXNAT   =0
!
...
! Calculate target axis A3_TF in Target Frame
...
! 14.10.16 (BTD)
!*** diagnostic
!         write(0,*)'DDSCAT ckpt 8.5'
!         write(0,*)'   nat0,nat,nat3=',nat0,nat,nat3
!***
         CALL EXTEND(CMDFFT,ICOMP,ISCR1,IDVERR,IDVOUT,IOCC,IXYZ0(1,1),     &
                     IXYZ0(1,2),IXYZ0(1,3),BETADF,PHIDF,THETADF,SCRRS2,X0, &
                     MXNX,MXNY,MXNZ,MXNAT,NAT,NAT0,NAT3,NX,NY,NZ,IPNF)
!*** diagnostic
!         write(0,*)'DDSCAT ckpt 8.6'
!         write(0,*)'   nat0,nat,nat3=',nat0,nat,nat3
!***
...
!                     as PETRKP
         ...
         ELSEIF(CMDSOL=='QMRCCG')THEN
            NLAR=10
#ifdef DDSCAT_CUDA_SOLVERS
         ELSEIF(CMDSOL=='BICGS2')THEN
            ! CUDA BiCGStab(2), IFDDA-derived alias of the resident BCGS2 path.
            ! CXSC is not used as iterative GPU workspace, but DDSCAT still
            ! needs host scratch for polarization bookkeeping.
            NLAR=10
         ELSEIF(CMDSOL=='BICGS4')THEN
            NLAR=12
         ELSEIF(CMDSOL=='GPBGS2')THEN
            NLAR=12
         ELSEIF(CMDSOL=='GPBGS4')THEN
            NLAR=12
#endif
         ELSE
...
!-----------------------------------------------------------------------
! 150929 (BTD) move this to where it will be executed by slave processes
...
! 16.08.25 as noted by Sergey Ph. Ruzankin, POSWAVE is needed by each MPI
!          process, therefore need to broadcast it

      CALL MPI_BCAST_INT(POSWAVE,1,0,IERR)   ! 16.08.25 added
...
!*** diagnostic
!      write(0,*)'ddscat ckpt 10.5, myid=',myid,' MXN3=',MXN3
!***
      ALLOCATE(CXALPH(MXN3))
!*** diagnostic
!      write(0,*)'ddscat ckpt 10.6, myid=',myid
!***
...
! 2013.12.28 (BTD) added NRFLD to arg list of SHARE1
!                  following suggestion from Sergey Ruzankin
!*** diagnostic
...
!*** 
      CALL SHARE1(A1_TF,A2_TF,A3_TF,AEFFA,AK1,BETA,BETADF,BETMID,BETMXD,      &
                  ...
                  NCOMP,NORI,NORICHAR,NPHI,NRAD,NRFLD,NSCAT,NSMELTS,          &
                  NTHETA,NWAV,NX,NY,NZ,ORDERM,ORDERN,PHI,PHIDF,PHIMID,        &
                  PHIMXD,PHIN,PYD,PYDDX,PZD,PZDDX,SHPAR,SMIND1,SMIND2,        &
                  THETA,THETADF,THETAN,THTMID,THTMXD,TOL,WAVEA,WGTA,WGTB,     &
                  X0,XMAX,XMIN,YMAX,YMIN,ZMAX,ZMIN)                           !
...
!            write(0,*)'DDSCAT ckpt 14.71, norichar=',norichar
            write(0,*)'DDSCAT ckpt 14.71: cflpol1=',cflpol1
!***
            OPEN(UNIT=22,FILE=CFLPOL1,ACCESS='STREAM')
!*** diagnostic
            write(0,*)'DDSCAT ckpt 14.711'
!***
            READ(22,POS=POSWAVE)WAVE
!*** diagnostic
            write(0,*)'DDSCAT ckpt 14.712'
!***
            INQUIRE(UNIT=22,POS=POSAEFF)
!*** diagnostic
            write(0,*)'DDSCAT ckpt 14.713'
!***
            CLOSE(22)
         ELSE
            WAVE=WAVEA(IWAV)
...
!*** diagnostic
!         write(0,*)'DDSCAT ckpt 14.72'
!***

         IF(MYID==0)THEN

!*** diagnostic
!            write(0,*)'DDSCAT ckpt 14.73'
!            write(0,*)'  wave=',wave
...
!               write(0,*)'DDSCAT ckpt 14.81, norichar=',norichar
               write(0,*)'DDSCAT ckpt 14.81: cflpol1=',cflpol1
!***
               OPEN(UNIT=22,FILE=CFLPOL1,ACCESS='STREAM')
!*** diagnostic
               write(0,*)'DDSCAT ckpt 14.82'
!***
               READ(22,POS=POSAEFF)AEFF
!*** diagnostic
               write(0,*)'DDSCAT ckpt 14.83'
!***
               INQUIRE(UNIT=22,POS=POSAK_TF)
!*** diagnostic
               write(0,*)'DDSCAT ckpt 14.84'
!***
               CLOSE(22)
            ELSE
               AEFF=AEFFA(IRAD)
...
!                     write(0,*)'DDSCAT ckpt 15.3, norichar=',norichar
                     write(0,*)'DDSCAT ckpt 15.3: cfpol1=',cflpol1
!***
                     OPEN(UNIT=22,FILE=CFLPOL1,ACCESS='STREAM')
!                     READ(22)   ! NRWORD,...
...
!                     READ(22)   ! AEFF
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.4, POSAK_TF=',POSAK_TF
!***
                     READ(22,POS=POSAK_TF)AK_TF
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.5'
!***
                     INQUIRE(UNIT=22,POS=POSCXE0)
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.6'
!***
                     READ(22)CXE01_TF
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.7'
!***
                     IF(IORTH==2)THEN
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.75, CLFPOL2=',CFLPOL2
!***
                        OPEN(UNIT=23,FILE=CFLPOL2,ACCESS='STREAM')
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.8, POSCXE0=',POSCXE0
!***
                        READ(23,POS=POSCXE0)CXE02_TF
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 15.9'
!***
                      ENDIF
                  ENDIF
...
!                  write(0,fmt='(a,3f10.5)')' with a1_TF=',a1_tf
!                  write(0,fmt='(a,3f10.5)')'      a2_TF=',a2_tf
!                  write(0,fmt='(a,f10.5)')'    beta(1)=',beta(1)
!                  write(0,fmt='(a,f10.5)')'   theta(1)=',theta(1)
!                  write(0,fmt='(a,f10.5)')'     phi(1)=',phi(1)
!***
                  CALL ROTATE(A1_TF,A2_TF,AK1,CXE01_LF,CXE02_LF,BETA(IBETA), &
                              THETA(ITHETA),PHI(IPHI),EN0_TF,CXE01_TF,       &
                              CXE02_TF,MXSCA,MYID,NSCAT,ENSC_LF,EM1_LF,      &
                              EM2_LF,AKS_TF,EM1_TF,EM2_TF,XL_TF,YL_TF,ZL_TF) !

!*** diagnostic
!                  write(0,*)'DDSCAT ckpt 17'
!                  write(0,fmt='(a,f10.6)')' beta(deg)=',betad
!                  write(0,fmt='(a,f10.6)')'theta(deg)=',thetad
!                  write(0,fmt='(a,f10.6)')'  phi(deg)=',phid
!                  write(0,fmt='(a,3f10.6)')'     xl_tf=',xl_tf
!                  write(0,fmt='(a,3f10.6)')'     yl_tf=',yl_tf
!                  write(0,fmt='(a,3f10.6)')'     zl_tf=',zl_tf
!                  write(0,fmt='(a,3f10.6)')'    en0_tf=',en0_tf
!                  write(0,fmt='(a,6f10.6)')'  cxe01_tf=',cxe01_tf
!                  write(0,fmt='(a,6f10.6)')'  cxe02_tf=',cxe02_tf
!                  write(0,fmt='(a,3f10.6)')'     a1_tf=',a1_tf
!                  write(0,fmt='(a,3f10.6)')'     a2_tf=',a2_tf
!                  write(0,fmt='(a,3f10.6)')'a1tf*en0tf=',                     &
!                     (a1_tf(1)*en0_tf(1)+a1_tf(2)*en0_tf(2)+a1_tf(3)*en0_tf(3))
!                  write(0,fmt='(a,3f10.6)')'a2tf*en0tf=',                     &
!                     (a2_tf(1)*en0_tf(1)+a2_tf(2)*en0_tf(2)+a2_tf(3)*en0_tf(3))
!                  write(0,fmt='(a,f10.6)')' a1tf*a2tf=',                   &
!                     (a1_tf(1)*a2_tf(1)+a1_tf(2)*a2_tf(2)+a1_tf(3)*a2_tf(3))
!***
...
!                        ENSC_LF,EM1_LF,EM2_LF in LF
                        ...
                        CALL PBCSCAVEC(MXSCA,JPBC,NSCAT,PYDDX,PZDDX,A1_TF,     &
                                       A2_TF,THETA(ITHETA),BETA(IBETA),XL_TF,  &
                                       YL_TF,ZL_TF,AK1,EN0_TF,CXE01_TF,ORDERM, &
                                       ORDERN,THETAN,PHIN,AKS_TF,EM1_TF,       &
                                       EM2_TF,ENSC_LF,EM1_LF,EM2_LF)
                     ENDIF
                  ENDIF
...
!                     write(0,*)'DDSCAT ckpt 18.21, norichar=',norichar
                     write(0,*)'DDSCAT ckpt 18.21: cflpol1=',cflpol1
!***
                  ELSE
...
!                     write(0,*)'DDSCAT ckpt 18.23, norichar=',norichar
                     write(0,*)'DDSCAT ckpt 18.23: cflpol1=',cflpol1
!***
                  ENDIF
...
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 18.3'
!***
                     READ(22)CXSC(1:3*NAT0)
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 18.4'
!***
                     READ(22)CXADIA(1:3*NAT0)
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 18.5'
!***
                     IF(IORTH==2)THEN
! read CXPOL2
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 18.6'
!***
                        READ(23)CXSCR1(LACE:LACE+3*NAT0-1)
!*** diagnostic
                     write(0,*)'DDSCAT ckpt 18.7'
!***
                     ENDIF
...
!*** diagnostic
!                     write(0,fmt='(a,i4,a,i4,a)')                    &
!                        'DDSCAT ckpt 21 myid=',myid,' NRFLD=',NRFLD, &
!                        '   about to call GETFML'                    !
!                     write(0,*)'           nat=',nat
...
!*** diagnostic
!			write(0,fmt='(a,i4,a,1p2e10.2)') &
!                           'DDSCAT ckpt 22, myid=',myid, &
!                           ' cxf11(7)=',cxf11(7)         !
!			write(0,fmt='(a,i4,a,1p2e10.2)') &
!                           'DDSCAT ckpt 22, myid=',myid, &
!                           ' cxf21(7)=',cxf21(7)         !
...
!***
                        ...
                        CALL WRITEFML(JPBC,IORI,IORTH,IRAD,IWAV,MXCOMP,MXSCA, &
                                      ...
                                      PHID,THETAD,TOL,WAVE,XX,A1_TF,A2_TF,    &
                                      PHIN,THETAN,CXE01_LF,CXE01_TF,CXE02_LF, &
...
!*** diagnostic
!               write(0,fmt='(a,i2,1p2e10.3)')                     &
!                 'DDSCAT ckpt 25 myid,sm(1,1,7),smori_1(1,1,7)=', &
!                 myid,sm(1,1,7),smori_1(1,1,7)                    !
!***
...
!**** If IWRKSC=1, write scattering properties of current orientation:
                     ...
                     IF(IWRKSC==1)THEN
                          ...
                          CALL WRITESCA(MXNX,MXNY,MXNZ,NX,NY,NZ,DX,ICOMP,     &
                             ...
                             THTMID,THTMXD,TOL,WAVE,XX,A1_TF,A2_TF,PHIN,QABS, &
...
! 200501 (BTD) change so that call WRITEPOL for all IPHI values
...
!*** diagnostic
                        write(0,*)'DDSCAT ckpt 27.1, write CFLPOL1=',CFLPOL1
!***
                        CALL WRITEPOL(NRWORD,MXNAT,NX,NY,NZ,NAT0,IANISO,     &
                                      ICOMP,ISCR1,IXYZ0,JPBC,PYD,PZD,GAMMA,  &
                                      NAMBIENT,A1_TF,A2_TF,AEFF,AK_TF,DX,X0, &
                                      WAVE,BETADF,THETADF,PHIDF,CXE01_TF,    &
                                      CXADIA,CXAOFF,CXSCR1,CFLPOL1)          !
!*** diagnostic
                        write(0,*)'DDSCAT ckpt 27.2, CFLPOL1=',CFLPOL1
!***
!*** diagnostic
...
! 120603 (BTD) correction: reversed ISCR1,ICOMP -> ICOMP,ISCR1 in arg list
                           ...
                           CALL WRITEPOL(NRWORD,MXNAT,NX,NY,NZ,NAT0,           &
                                         ...
                                         PYD,PZD,GAMMA,NAMBIENT,A1_TF,A2_TF,   &
                                         AEFF,AK_TF,DX,X0,WAVE,BETADF,THETADF, &
...
!*** diagnostic
!               write(0,fmt='(a,i2,1p2e10.3)')                        &
!                 'DDSCAT ckpt 32 myid,smori(1,1,7),smori_1(1,1,7)=', &
!                 myid,smori(1,1,7),smori_1(1,1,7)                    !
!***
...
!*** diagnostic
!               write(0,fmt='(a,i2,1p2e10.3)')                        &
!                 'DDSCAT ckpt 33 myid,smori(1,1,7),smori_1(1,1,7)=', &
!                 myid,smori(1,1,7),smori_1(1,1,7)                    !
!***
...
!******** Now print out orientational averages **********
               ...
               IF(MYID==0)THEN
                  ...
                  CALL WRITESCA(MXNX,MXNY,MXNZ,NX,NY,NZ,DX,ICOMP,NAMBIENT,     &
                          ...
                          PHIMXD,THETAD,THTMID,THTMXD,TOL,WAVE,XX,A1_TF,A2_TF, &
...
9011  FORMAT(6X,I8,                                                     &
         ' = NAT  = number of lattice sites in extended target'/7X,3I4, &
         ' = x,y,z length of extended target (Targ. Frame)')            !
9020  FORMAT(' DDSCAT --- ',A/' TARGET ---',A/' ',A,                      &
         ' --- method of solution '/' ',A,                                &
         ' --- prescription for polarizabilities'/' ',A,' --- shape '/I8, &
...
9048  FORMAT('  wave(um)   f(cm-1)     Re(m)     Im(m)     Re(eps)   ', &
        'Im(eps)')                                                      !
9058  FORMAT(1P,E11.4,E11.4,2E10.3,E11.3,E10.3)
